				...
				do
					...
					do
						...
						do
							...
							do
								python3 cnn_hpc_final.py --num_hidden=$num_hidden --kernel_size=$kernel_size --pred_seq_len=$pred_seq_len --loss=$loss --weight_decay=$weight_decay --dropout=$dropout --case=$case
							done
						done
					done
				done
